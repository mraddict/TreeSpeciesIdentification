...
public:
	...
	static ReprojectReport reproject(const std::string& srcPath, const std::string& dstPath, const ReprojectOptions& opts)
	{
		GDALAllRegister();
		...
		std::cout << "=== 좌표계 변환 시작 ===\n" << "  원본: EPSG:" << report.srcEpsg << " (" << report.srcWidth << "x" << report.srcHeight << ", " << nBands << "밴드)\n" << "  대상: EPSG:" << report.dstEpsg << "\n";

		// ── 3. 대상 영역 및 해상도 계산 ──
		double dstGT[6];
		int dstWidth, dstHeight;
		computeTargetExtent(srcDs, srcWkt, dstWkt, opts.dstResX, opts.dstResY, dstGT, dstWidth, dstHeight);
		...
		std::cout << "  변환 후 크기: " << dstWidth << "x" << dstHeight << "\n"
			...
			<< report.dstMinX << " ~ " << report.dstMaxX << "] Y[" << report.dstMinY << " ~ " << report.dstMaxY << "]\n";

		// ── 4. 출력 파일 생성 ──
		GDALDriver* driver = GetGDALDriverManager()->GetDriverByName(opts.format.c_str());
		...
		GDALDataset* dstDs = driver->Create(dstPath.c_str(), dstWidth, dstHeight, nBands, srcType, createOpts);
		...
		if (opts.numThreads > 1)
		{
			...
			warpOpts->papszWarpOptions = CSLSetNameValue(warpOpts->papszWarpOptions, "NUM_THREADS", std::to_string(opts.numThreads).c_str());
		}

		// 좌표 변환기 생성
		warpOpts->pTransformerArg = GDALCreateGenImgProjTransformer(srcDs, srcWkt, dstDs, dstWkt, FALSE, 0.0, 1);
	...
	// ─── 변환 결과 검증 ───
	static void verify(const std::string& dstPath, int expectedEpsg, double refMinX, double refMaxX, double refMinY, double refMaxY)
	{
		GDALAllRegister();
		...
		{
			std::cout << "  ✗ 좌표계 불일치! 기대: EPSG:" << expectedEpsg << " / 실제: EPSG:" << epsg << "\n";
		}
		...
		{
			double overlapArea = (overlapMaxX - overlapMinX) * (overlapMaxY - overlapMinY);
			double aerialArea = (dstMaxX - dstMinX) * (dstMaxY - dstMinY);
			double forestArea = (refMaxX - refMinX) * (refMaxY - refMinY);

			std::cout << "  ✓ 범위 겹침 확인\n"
				<< "    겹침 영역: X[" << overlapMinX << " ~ " << overlapMaxX << "] Y[" << overlapMinY << " ~ " << overlapMaxY << "]\n"
				<< "    항공사진 대비: " << std::setprecision(1) << (overlapArea / aerialArea * 100.0) << "%\n"
				<< "    임상도 대비  : " << (overlapArea / forestArea * 100.0) << "%\n";
		}
		else
		{
		...
		// 해상도
		std::cout << std::setprecision(4) << "  해상도: " << gt[1] << " m/pixel\n" << "  크기  : " << w << " x " << h << " pixels\n";
...
private:
	...
	// 대상 영역 및 해상도 자동 계산
	static void computeTargetExtent(GDALDataset* srcDs, const char* srcWkt, const char* dstWkt, double reqResX, double reqResY, double* dstGT, int& dstWidth, int& dstHeight)
	{
		...
		OGRCoordinateTransformation* ct = OGRCreateCoordinateTransformation(&oSrcSRS, &oDstSRS);

		if (!ct)
		{
			throw std::runtime_error("좌표 변환기 생성 실패 (EPSG:" + std::to_string(0) + " → EPSG:" + std::to_string(0) + ")");
		}
		...
		if (resX <= 0.0 || resY <= 0.0)
		{
			...
			OGRCoordinateTransformation* ct2 = OGRCreateCoordinateTransformation(&s1, &s2);
	...
	static GDALResampleAlg parseResampleAlg(const std::string& name)
	{
		...
		std::cerr << "  ⚠ 알 수 없는 리샘플링 '" << name << "' → bilinear 사용\n";
